...
* Classes
*  
* Low level class for manipulating the data in the table classes
*
...
class Classes {
	...
	*/
	...
	/**
	* private class variable $class_name
	*/
	private $_className;
	...
	*
	* @return object(int|string) className
	*/
	public function _getclass_name() {
		return $this->_className;
	}
	
	/**
	* sets the value of $_className
	*
	* @param className
	*/
	public function _setclass_name($className) {
	...
	* @return object ( this class)
	*/
	public function setclass_name($className) {
		$this->_setclass_name($className);
		return $this;
	}
	...
	*/
     ...
     * based on the value of $school_id,$class_id,$class_name,$create_time,$last_modified,$trashed passed to the function
     *
     * @param $school_id,$class_id,$class_name,$create_time,$last_modified,$trashed
     * @return object (class_id)| null
     */
	public function getclass_id($school_id,$class_id,$class_name,$create_time,$last_modified,$trashed) {
		...
		$class_id_ = $this->query_from_classes ( $columns, $records );
     ...
     * based on the value of $class_id passed to the function
     *
     * @param $class_id
     * @return object (school_id)| null
     */
	public function getschool_id($class_id) {
		$columns = array ('class_id');
		$records = array ($class_id);
		$school_id_ = $this->query_from_classes ( $columns, $records );
		return sizeof($school_id_)>0 ? $school_id_ [0] ['school_id'] : null;
	}
	
	
	/**
     * Performs a database query and returns the value of class_name 
     * based on the value of $class_id passed to the function
     *
     * @param $class_id
     * @return object (class_name)| null
     */
	public function getclass_name($class_id) {
		$columns = array ('class_id');
		$records = array ($class_id);
		$class_name_ = $this->query_from_classes ( $columns, $records );
		return sizeof($class_name_)>0 ? $class_name_ [0] ['class_name'] : null;
	}
     ...
     */
	public function getcreate_time($class_id) {
		$columns = array ('class_id');
		$records = array ($class_id);
		$create_time_ = $this->query_from_classes ( $columns, $records );
     ...
     */
	public function getlast_modified($class_id) {
		$columns = array ('class_id');
		$records = array ($class_id);
		$last_modified_ = $this->query_from_classes ( $columns, $records );
     ...
     */
	public function gettrashed($class_id) {
		$columns = array ('class_id');
		$records = array ($class_id);
		$trashed_ = $this->query_from_classes ( $columns, $records );
	...
	*/
	public function insert_prepared_records($school_id,$class_id,$class_name,$create_time,$last_modified,$trashed,$redundancy_check= false, $printSQL = false) {
		$columns = array('school_id','class_id','class_name','create_time','last_modified','trashed');
		$records = array($school_id,$class_id,$class_name,$create_time,$last_modified,$trashed);
		return $this->insert_records_to_classes ( $columns, $records,$redundancy_check, $printSQL );
	}
	...
	* @return string table name -> 'classes' 
	*/
	...
	*/
	...
	/**
     * Used  to calculate the number of times a record exists in the table classes
     * It returns the number of times a record exists exists in the table classes
     * @param array $columns
     * @param array $records
     * @param bool $printSQL
     * @return mixed
	...
	/**
     * Inserts data into the table classes
     * if redundancy check is true, it inserts if the record if it never existed else.
     * if the record exists, it returns the number of times the record exists on the relation
     *
     * @param array $columns
	...
	/**
	 * Deletes all the records that meets the passed criteria from the table classes
	 * @param array $columns
	 * @param array $records
	 * @param bool $printSQL
	 * @return number of deleted rows
	 */
	public function delete_record_from_classes(Array $columns, Array $records, $printSQL = false) {
		return $this->delete_record ( $this->get_table (), $columns, $records, $printSQL );
	}
	
	/**
	 * Updates all the records that meets the passed criteria from the table classes
	 * 
	 * @param array $columns
	 * @param array $records
	 ...
	 */
	public function update_record_in_classes(Array $columns, Array $records, Array $where_columns, Array $where_records, $printSQL = false) {
		return $this->update_record ( $this->get_table (), $columns, $records, $where_columns, $where_records, $printSQL );
	}
	
	/**
	 * Gets an Associative array of the records in the table 'classes' that meets the passed criteria
	 *
	 * @param $distinct
	 * @param array $columns
	 * @param array $records
	 ...
	 */
	public function fetch_assoc_in_classes($distinct, Array $columns, Array $records, $extraSQL="", $printSQL = false) {
	 ...
	 * @return array|mixed associative
	 */
	public function query_from_classes(Array $columns, Array $records,$extraSQL="",  $printSQL = false) {
		return $this->query ( $this->get_table (), $columns, $records,$extraSQL,$printSQL );
	}
	
	/**
	 * Gets an Associative array of the records in the table classes that meets the passed distinct criteria
	 * 
	 * @param array $columns
	 * @param array $records
	 * @param string $extraSQL
	 ...
	 * @return array|mixed associative
	 */
	public function query_distinct_from_classes(Array $columns, Array $records,$extraSQL="",  $printSQL = false) {
		return $this->query_distinct ( $this->get_table (), $columns, $records,$extraSQL,$printSQL );
	}
	
	/**
	 * Performs a search in the table classes that meets the passed criteria
	 * 
	 * @param array $columns
	 * @param array $records
	 * @param string $extraSQL
	 ...
	 * @return array|mixed associative
	 */
	public function search_in_classes(Array $columns, Array $records,$extraSQL="", $printSQL = false) {
   ...
   * @throws NullabilityException
	...
	/**
     * Gets an Associative array of the records in the table classes that meets the distinct passed criteria
     * @param $table
     * @param array $columns
     * @param array $records
     * @param string $extraSQL
	...
	private function query_distinct($table, Array $columns, Array $records,$extraSQL="",$printSQL = false) {
	 ...
	 /**
     * Performs a search and returns an associative array of the records in the table classes  that meets the passed criteria
     * 
     * @param $table
     * @param array $columns
     * @param array $records
